...
def QPlot(Title, Set, Points, Regions, Data, PlotRanges, xylims): # This function generates the 2x2 plots containing the information gathered in Theo_Gen.
    ...
    plt.xlabel("C1 / C2", fontsize = 10)
    plt.ylabel("C2 / C3", fontsize = 10)
    ...
    if Points == True:
        ax[0,0].set_title(f"{PlotRanges[0]} <= cmax <= {PlotRanges[1]}", fontsize = 10)
    else:
        ax[0,0].set_title(f"CScale = {int((PlotRanges[0] + PlotRanges[1]) / 2)}", fontsize = 10)
    ax[0,1].contourf(Data["T_c1ratio"], Data["T_c2ratio"], Data['Bin_2'][Set].T, levels = 10, vmin = Data[Set + '_min'], vmax = Data[Set + '_max'])
    if Points == True:
        ax[0,1].set_title(f"{PlotRanges[1]} <= cmax <= {PlotRanges[2]}", fontsize = 10)
    else:
        ax[0,1].set_title(f"CScale = {int((PlotRanges[1] + PlotRanges[2]) / 2)}", fontsize = 10)
    ax[1,0].contourf(Data["T_c1ratio"], Data["T_c2ratio"], Data['Bin_3'][Set].T, levels = 10, vmin = Data[Set + '_min'], vmax = Data[Set + '_max'])
    if Points == True:
        ax[1,0].set_title(f"{PlotRanges[2]} <= cmax <= {PlotRanges[3]}", fontsize = 10)
    else:
        ax[1,0].set_title(f"CScale = {int((PlotRanges[2] + PlotRanges[3]) / 2)}", fontsize = 10)
    ax[1,1].contourf(Data["T_c1ratio"], Data["T_c2ratio"], Data['Bin_4'][Set].T, levels = 10, vmin = Data[Set + '_min'], vmax = Data[Set + '_max'])
    if Points == True:
        ax[1,1].set_title(f"cmax > {PlotRanges[3]}", fontsize = 10)
    else:
        ax[1,1].set_title(f"CScale = {int((PlotRanges[3] + PlotRanges[4]) / 2)}", fontsize = 10)
...
def RangeGraphs(Regions, Data, siteNames, GroupLabel): # This function generates the range plot for bird-plant interactions.
    ...
    fig.suptitle('Range Plot of Variance by Region', fontsize = 20)
    fig.set_size_inches(12, 9)
    
    fig.add_subplot(111, frameon = False)
    plt.tick_params(labelcolor='none', which='both', top=False, bottom=False, left=False, right=False)
    plt.xlabel("Sorted "+ GroupLabel + " Index", fontsize = 15)
    plt.ylabel("Variance", fontsize = 15)
    
    for j in range(2):
        for i in range(3): # Iterates through each site
            Reg = siteNames[i,j]
            ax[i,j].set_title(Reg, fontsize = 15)
            ...
            # ax[i,j].set_title(Reg) # Redundant? I think so.
...
def cmax_Hist(Regions, siteNames, Group):
    fig, ax = plt.subplots(3,2)
    fig.suptitle('Maximum interaction counts', fontsize = 20)
    fig.set_size_inches(12, 9)
    plt.rcParams.update({'font.size': 8})

    fig.add_subplot(111, frameon = False)
    plt.tick_params(labelcolor='none', which='both', top=False, bottom=False, left=False, right=False)
    plt.xlabel("Maximum number of interactions per location", fontsize = 15)

    for j in range(2):
        for i in range(3):
            ax[i,j].hist(np.log10(Regions[siteNames[i,j]]["cmax"][Regions[siteNames[i,j]]["cmax"] != 0]), bins = np.linspace(0,3,13))
            ax[i,j].set_title(siteNames[i,j], fontsize = 15)
            ax[i,j].set_xlim([0,3])
...
def ERange_Plot(Regions, siteNames):
    fig, ax = plt.subplots(2,2)
    fig.suptitle('Inter-location Interaction Strength Variance by Genus, Species-Combined and Species-Separated', fontsize = 20)
    fig.set_size_inches(12, 9)
    plt.rcParams.update({'font.size': 8})
    ...
    for j in range(2):
        for i in range(2):
            ...
            ax[i,j].set_title(siteNames[i,j], fontsize = 15)
            ax[i,j].set_xticks(range(xRange), keys, fontsize = 10)
    ...
    labels, index = np.unique(labels, return_index=True)
    handles = [handles[i] for i in index]
    ax[1,1].legend(handles, labels, loc = 'lower right', fontsize = 15)
    
    plt.ylabel("Variance", fontsize = 15, labelpad = 10)
...
def ClusteredStackedBarChart(Regions, siteNames):
    fig, ax = plt.subplots(2,2)
    fig.suptitle('Interaction Counts of each Genus, Separated by Species and Site', fontsize = 20)
    fig.set_size_inches(12, 9)
    plt.rcParams.update({'font.size': 8})
    ...
    for j in range(2):
        for i in range(2):
            ...
            ax[i,j].set_xticklabels(genus_names, fontsize = 10)
            ax[i,j].tick_params(labelrotation = 45)
            
    fig.supylabel("Interaction Counts", fontsize = 15)
    fig.supxlabel("Genus-Genus Combinations", fontsize = 15)
